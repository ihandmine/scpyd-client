...
from aioscpy.utils.tools import get_config

from scpyd_client import commands
from scpyd_client.utils import ErrorResponse, MalformedRespone
...
def parse_cli_args(args):
    ...
    target_default = cfg.get(
        'deploy', 'url', fallback=DEFAULT_TARGET_URL).rstrip('/')
    ...
    parser = subparsers.add_parser(
        'deploy', description=commands.deploy.__doc__)
    parser.set_defaults(action=commands.deploy)

    parser = subparsers.add_parser(
        'projects', description=commands.projects.__doc__)
    parser.set_defaults(action=commands.projects)

    parser = subparsers.add_parser(
        'schedule', description=commands.schedule.__doc__)
    ...
    parser = subparsers.add_parser(
        'spiders', description=commands.spiders.__doc__)
...
def main():
    try:
        ...
        print(
            f'Caught unhandled exception, please report at owner github issue')
        print_exc()
        exit_code = 3
    else:
        exit_code = 0
